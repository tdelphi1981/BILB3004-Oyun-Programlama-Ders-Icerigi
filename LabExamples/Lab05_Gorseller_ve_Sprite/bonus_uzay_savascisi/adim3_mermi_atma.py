"""
Uzay Savaşçısı - Adım 3: Mermi Atma

SPACE tuşu ile mermi atan bir sistem ekler.
Mermi, oyuncunun üstünden çıkarak yukarı hareket eder.
Ekrandan çıkan mermiler otomatik silinir (kill).

Öğrenilecek kavramlar:
- Yeni Sprite sınıfı oluşturma (Mermi)
- KEYDOWN olayı ile mermi oluşturma
- Sprite.kill() ile otomatik temizlik
- Ateş hız sınırı (cooldown) zamanlama
- Birden fazla sprite grubu yönetimi

Bölüm: 05 - Görseller ve Sprite Temelleri
Lab: 05 - Bonus: Uzay Savaşçısı (Adım 3/7)

Çalıştırma: uv run python adim3_mermi_atma.py
...
MERMI_HIZ = 8
MERMI_BOYUT = (9, 37)
MERMI_BEKLEME = 250  # ms (ateşler arası minimum süre)
...
class Oyuncu(pygame.sprite.Sprite):
    ...
    def ates_et(self, mermi_grubu, tum_grup):
        ...
        Args:
            mermi_grubu: Mermilerin eklendiği grup.
            tum_grup: Tüm sprite'ların eklendiği grup.

        Returns:
            True: Mermi atıldıysa, False: Cooldown bekleniyor.
        """
...
class Mermi(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y):
        ...
        Args:
            ...
            y: Başlangıç y konumu (üst kenar).
        """
        super().__init__()
...
Ekranın alt ortasında mavi uzay gemisi görülür.

WASD/Ok tuşları ile gemiyi hareket ettir.
SPACE tuşuna basınca geminin üstünden mavi mermi çıkar
ve yukarı doğru hızla ilerler.

Mermiler ekranın üst kenarını geçince otomatik silinir.
Sol üst köşede aktif mermi sayısı görülür.
Ateşler arası minimum 250ms bekleme süresi vardır.
